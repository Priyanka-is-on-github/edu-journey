...
const ChapterActions = ({
  ...
  const onClick = async () => {
    try {
      ...
      if (ispublished) {
        ispublished = false;
        await fetch(
          `http://localhost:3001/api/v1/courses/chapterdetail?chapterId=${params.chapterid}&ispublish=${ispublished}&courseId=${params.id}`,
          {
            method: "PUT",
            headers: {
  ...
  const onDelete = async () => {
    try {
      setIsLoading(true);
      await fetch(
        `http://localhost:3001/api/v1/courses/chapterdelete/${params.chapterid}`,
        {
          method: "DELETE",
          headers: {
            "Content-type": "application/json",
